...
describe(
  ...
  () => {
    ...
    it('시스템관리 > 수동발송 메뉴로 이동한다.', async() => {
      const GRAND_PARENT_NODE = "div.ikc-gnb-menus > div.ikc-gnb-item > ul > li > button[aria-label='시스템관리']"
    })
  },
  timeout
)
...
async function moveMenuOfThreeDepth(page, grandParentLabel, parentLabel, grandChildHref) {
  await page.click('div.ikc-gnb-menus > div.ikc-gnb-item > ul > li > button[aria-label="' + grandParentLabel + '"]');
  await page.click('div.ikc-gnb-menus > div.ikc-gnb-item > ul > li > ul > li > a[href="#' + childHref + '"]');
	await page.click('div.ikc-gnb-menus > div.ikc-gnb-item > ul > li > ul > li > a[href="#' + grandChildHref + '"]');
}
*/
